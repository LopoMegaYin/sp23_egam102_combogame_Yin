...
namespace MicroCombo 
{
    public static class UtilsMath
    {
        ...
        public static float EvaluateCurve(float interp, AnimationCurve curve)
        {
            float ret = interp;
            if (curve != null &&
                curve.length > 0)
            {
                ret = curve.Evaluate(interp);
            }
            return ret;
        }

        public static int Mod(float val, int mod)
        {
            return Mod(Mathf.FloorToInt(val), mod);
        }

        public static int Mod(int val, int mod)
        {
            return Mathf.FloorToInt(Mod((float)val, (float)mod));
        }

        public static float Mod(float val, float mod)
        {
            // C# doesn't do negative mods as expected - so get it to be positive first!
            while (val < 0 &&
                mod > 0)
            {
                val += mod;
        ...
        public static int Size<T>() where T : Enum
        {
